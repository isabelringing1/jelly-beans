...
import { Fn, If, Return, instancedArray, instanceIndex, uniform, attribute, uint, float, clamp, struct, atomicStore, int, ivec3, array, vec3, atomicAdd, Loop, atomicLoad, max, pow, mat3, vec4, cross, step, range, mod, color, hash } from 'three/tsl';
...
let randomIndexUniform, randomFallPositionUniform;
...
function setupUniforms() {
    ...
    stiffnessUniform = uniform( 1000 );
    restDensityUniform = uniform( 1.5 );
    dynamicViscosityUniform = uniform( 5 );
    dtUniform = uniform( 1 / 60 );
    gravityUniform = uniform( new THREE.Vector3( 0, params.gravityPull, 0 ) );
    mouseRayOriginUniform = uniform( new THREE.Vector3( 0, 0, 0 ) );
    mouseRayDirectionUniform = uniform(new THREE.Vector3(0, 0, 0));
    randomIndexUniform = uniform(-1);
    randomFallPositionUniform = uniform(new THREE.Vector3(.5, 1, 0.5));
...
function setupComputeShaders() {
    ...
    p2g2Kernel = Fn( () => {
        ...
        const cellIndex = ivec3( gridPosition ).sub( 1 ).toConst( 'cellIndex' );
        const cellDiff = gridPosition.fract().sub( 0.5 ).toConst( 'cellDiff' );
        const w0 = float( 0.5 ).mul( float( 0.5 ).sub( cellDiff ) ).mul( float( 0.2 ).sub( cellDiff ) ); //change second float from 0.5 to 0.2
        const w1 = float( 0.75 ).sub( cellDiff.mul( cellDiff ) );
        const w2 = float( 0.5 ).mul( float( 0.5 ).add( cellDiff ) ).mul( float( 0.2 ).add( cellDiff ) ); //change second float from 0.5 to 0.2
        const weights = array( [ w0, w1, w2 ] ).toConst( 'weights' );
    ...
    g2pKernel = Fn( () => {
        ...
        const innerBox = gridSizeUniform.mul( 0.3 ).sub( 9.0 ).div( gridSizeUniform ).toVar();
        ...
        */
        ...
        If(randomIndexUniform.greaterThanEqual(instanceIndex).and(randomIndexUniform.lessThan(instanceIndex.add(22))), () => {
            
            var randX = float(0.5).add(hash(instanceIndex).sub(0.5).mul(0.2));
            var randY = float(1.0).add(hash(instanceIndex).sub(0.5).mul(-0.2))
            var randZ = float(0.5).add(hash(instanceIndex).sub(0.5).mul(.2));
            particleVelocity.assign(vec3(0, 0, 0));
            particlePosition.assign(vec3(randX, randY, randZ));
        });
...
function setupMesh() {
    ...
    var capsule = new THREE.CapsuleGeometry(0.005, 0.005, 4, 8).deleteAttribute('uv');
    
    const geometry = BufferGeometryUtils.mergeVertices(capsule);

    //geometry.rotateX(Math.random(0, 3))

    const material = new THREE.MeshPhongMaterial({ shininess: 4, specular: new THREE.Color( 0xfff000 )})

    material.positionNode = Fn(() => {
        var particlePosition = particleBuffer.element(instanceIndex).get('position');
        return attribute('position').add(particlePosition);
    })();

    /*material.geometryNode = Fn(({ renderer, geometry, object }) => {
        const particleColors = array([ 1.0, 2.0, 3.0 ]).toVar();
        return particleColors.element(instanceIndex).toVar();
    })();*/
 

    material.colorNode = getRandomColor();
...
const getRandomRotation = Fn(() => {
    const seed = range(0, 2).toVar();
    return uint(seed);
})

const getRandomColor = Fn(() => {
    const particleColors = array([
        color(211 / 255, 30 / 255, 37 / 255),
        color(255 / 255, 235 / 255, 209 / 255),  
        color(58 / 255, 138 / 255, 68 / 255),    
        color(151 / 255, 202 / 255, 63 / 255),    
        color(1 / 255, 86 / 255, 143 / 255),
        color(242 / 255, 128 / 255, 47 / 255), //orange
        color(255 / 255, 184 / 255, 201 / 255), //pink
        color(111 / 255, 51 / 255, 22 / 255) // brown
    ]).toVar();

    const seed = range(0, 8).toVar();

    return particleColors.element(seed);
});
...
function setupMouse() {
    ...
    const onClick = (e) => {
        const myVal = particleBuffer.element(1).get('position');
        console.log(myVal)
        var random = Math.floor(Math.random() * 1000)
        randomIndexUniform.value = random;
    }
...
async function render() {
    ...
    mouseForceUniform.value.copy( mouseCoord ).sub( prevMouseCoord ).multiplyScalar( 0.5 );
    ...
    var random = Math.floor(Math.random() * 1000)
    randomIndexUniform.value = random;
    randomFallPositionUniform.value = vec3(.5 + (Math.random() - 0.5), 1, .5 + (Math.random() - 0.5));
